...
class SubtitleServiceTest : ServiceTest<SubtitleService>() {
    ...
    @Test
    @Throws(IOException::class)
    fun `Most downloaded response is parsing`() = runBlocking {
        enqueueResponse("search/response.json")
        val expected = SubtitleMocks.searchResponse
        val actual = service.getMostDownloaded()
        mockWebServer.takeRequest()
    ...
    @Test
    @Throws(IOException::class)
    fun `Latest subtitles response parsing`() = runBlocking {
        enqueueResponse("search/response.json")
        val expected = SubtitleMocks.searchResponse
        val actual = service.getLatestSubtitles()
        mockWebServer.takeRequest()
